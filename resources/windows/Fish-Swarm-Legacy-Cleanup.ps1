...
function Write-Step {
  ...
  Write-Host "[FishSwarm Legacy Cleanup] $Message"
}
...
function Get-FishSwarmRegistryEntries {
  $registryGlobs = @(
  ...
  foreach ($glob in $registryGlobs) {
    ...
    $items = Get-ItemProperty -Path $glob -ErrorAction SilentlyContinue | Where-Object {
      $_.DisplayName -like "FishSwarm*" -or
        $_.DisplayName -like "Fish Swarm*" -or
        $_.Publisher -eq "FishSwarm Team" -or
        $_.Publisher -eq "Fish Swarm Team"
    }
...
function Stop-FishSwarmProcesses {
  $processes = @(Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | Where-Object {
    $_.Name -ieq "FishSwarm.exe" -or
      $_.Name -ieq "Fish Swarm.exe" -or
      $_.ExecutablePath -like "*\FishSwarm.exe" -or
      $_.ExecutablePath -like "*\Fish Swarm.exe"
  })

  if ($processes.Count -eq 0) {
    Write-Step "No running FishSwarm processes found."
    return
  }
...
$registryEntries = @(Get-FishSwarmRegistryEntries)
...
Add-UniquePath -List $installPaths -PathValue (Join-Path $env:LOCALAPPDATA "Programs\FishSwarm")
Add-UniquePath -List $installPaths -PathValue (Join-Path $env:LOCALAPPDATA "Programs\Fish Swarm")

$appDataPaths = [System.Collections.Generic.List[string]]::new()
Add-UniquePath -List $appDataPaths -PathValue (Join-Path $env:APPDATA "FishSwarm")
Add-UniquePath -List $appDataPaths -PathValue (Join-Path $env:APPDATA "Fish Swarm")
Add-UniquePath -List $appDataPaths -PathValue (Join-Path $env:APPDATA "fishswarm")
Add-UniquePath -List $appDataPaths -PathValue (Join-Path $env:LOCALAPPDATA "FishSwarm")
Add-UniquePath -List $appDataPaths -PathValue (Join-Path $env:LOCALAPPDATA "Fish Swarm")
Add-UniquePath -List $appDataPaths -PathValue (Join-Path $env:LOCALAPPDATA "fishswarm")

Write-Host ""
Write-Step "This tool removes broken FishSwarm / legacy Fish Swarm Windows install leftovers."
...
if ($failures.Count -eq 0) {
  Write-Step "Cleanup finished. You can rerun the FishSwarm installer now."
