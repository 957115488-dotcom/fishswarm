...
describe('ConfigStore applyToEnv', () => {
  const originalEnv = {
    FISHSWARM_WORKDIR: process.env.FISHSWARM_WORKDIR,
  ...
  afterEach(() => {
    if (originalEnv.FISHSWARM_WORKDIR === undefined) {
      delete process.env.FISHSWARM_WORKDIR;
    } else {
      process.env.FISHSWARM_WORKDIR = originalEnv.FISHSWARM_WORKDIR;
    }
  ...
  it('clears stale FISHSWARM_WORKDIR when config value is removed', () => {
    const store = new ConfigStore();

    store.update({
      defaultWorkdir: '/tmp/fishswarm-valid-workdir',
      provider: 'anthropic',
      apiKey: 'sk-ant-test',
      model: 'claude-sonnet-4-5',
    });
    store.applyToEnv();
    expect(process.env.FISHSWARM_WORKDIR).toBe('/tmp/fishswarm-valid-workdir');
    ...
    expect(process.env.FISHSWARM_WORKDIR).toBeUndefined();
  });
