...
describe('api config state helpers', () => {
  ...
  it('keeps pristine custom openai profile in manual input mode', () => {
    ...
    expect(snapshot.profiles['custom:openai'].model).toBe('gpt-5.5');
  });

  it('exposes updated preset lists and custom guidance', () => {
    expect(FALLBACK_PROVIDER_PRESETS.openrouter.models.map((item) => item.id)).toContain(
      'openai/gpt-5.5-pro'
    );
    expect(FALLBACK_PROVIDER_PRESETS.openrouter.models.map((item) => item.id)).toContain(
      'openai/gpt-5.5'
    ...
    expect(FALLBACK_PROVIDER_PRESETS.openrouter.models.map((item) => item.id)).not.toContain(
      'openai/gpt-5.3-codex'
    );
    expect(FALLBACK_PROVIDER_PRESETS.openai.models.map((item) => item.id)).toContain('gpt-5.5');
    expect(FALLBACK_PROVIDER_PRESETS.openai.models.map((item) => item.id)).toContain('gpt-5.4');
    expect(FALLBACK_PROVIDER_PRESETS.openai.models.map((item) => item.id)).not.toContain(
      'gpt-5.3-codex'
    );
    expect(FALLBACK_PROVIDER_PRESETS.openai.models.map((item) => item.id)).not.toContain('o3');
    expect(FALLBACK_PROVIDER_PRESETS.openai.models.map((item) => item.id)).not.toContain('o4-mini');
    ...
    expect(FALLBACK_PROVIDER_PRESETS.anthropic.models.map((item) => item.id)).not.toContain(
    ...
    expect(getModelInputGuidance('custom', 'openai').placeholder).toContain('agnes-2.0-flash');
