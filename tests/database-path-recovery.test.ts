...
describe('database path recovery', () => {
  beforeEach(() => {
    testRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'fishswarm-db-path-test-'));
  });
  ...
  it('recovers a legacy sqlite file that occupies the data path', async () => {
    ...
    const recoveredDbPath = path.join(userDataPath, 'data', 'fishswarm.db');
  ...
  it('moves WAL sidecar files together with a recovered legacy database', async () => {
    ...
    expect(fs.readFileSync(path.join(userDataPath, 'data', 'fishswarm.db-wal'), 'utf8')).toBe('legacy-wal');
    expect(fs.readFileSync(path.join(userDataPath, 'data', 'fishswarm.db-shm'), 'utf8')).toBe('legacy-shm');
  ...
  it('backs up a conflicting plain file and creates a fresh database directory', async () => {
    ...
    const dbPath = path.join(userDataPath, 'data', 'fishswarm.db');
