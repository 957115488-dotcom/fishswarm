...
describe('provider guidance helpers', () => {
  ...
  it('detects Agnes and recommends OpenAI-compatible setup', () => {
    const setup = detectCommonProviderSetup('https://apihub.agnes-ai.com/v1');
    expect(setup?.id).toBe('agnes');
    expect(setup?.recommendedProtocol).toBe('openai');
    expect(setup?.exampleModel).toBe('agnes-2.0-flash');
  });
