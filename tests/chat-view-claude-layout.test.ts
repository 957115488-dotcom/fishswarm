...
describe('ChatView Claude-style layout', () => {
  ...
  it('uses a quieter header treatment with FishSwarm eyebrow and compact connector badge', () => {
    const source = readChatView();
    expect(source).toContain('FishSwarm');
  ...
  it('exposes configured model sets from the composer model switcher', () => {
    const source = readChatView();
    expect(source).toContain('configSets.map');
    expect(source).toContain('getConfigSetModel(configSet)');
    expect(source).toContain('window.electronAPI.config.switchSet');
    expect(source).toContain('modelConfigSets');
  });
});
