...
describe('getColdStartHistoryTokenBudget', () => {
  it('caps OpenAI-compatible cold-start history below common TPM limits', () => {
    expect(getColdStartHistoryTokenBudget('openai', 'openai', 128000)).toBe(12000);
    expect(getColdStartHistoryTokenBudget('custom', 'openai', 128000)).toBe(12000);
    expect(getColdStartHistoryTokenBudget('openrouter', 'openai', 128000)).toBe(12000);
  });

  it('keeps small local Ollama windows proportional instead of using the remote cap', () => {
    expect(getColdStartHistoryTokenBudget('ollama', 'openai', 8192)).toBe(1228);
  });
