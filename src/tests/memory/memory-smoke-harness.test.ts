...
function normalizeTextForAssert(value: string): string {
...
describe('memory smoke harness', () => {
  ...
  beforeEach(() => {
    storageRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'fishswarm-memory-smoke-'));
  ...
  it('simulates multi-session recall across same and different workspaces', async () => {
    ...
    expect(normalizeTextForAssert(otherWorkspacePrompt)).toContain('source=/repo/workspace-a');
