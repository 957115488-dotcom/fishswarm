...
vi.mock('electron', () => ({
  app: {
    ...
    getAppPath: () => '/tmp/fishswarm-test-app',
  },
}));
...
function normalizeWorkspaceForAssert(value: string | null | undefined): string | undefined {
...
describe('MemoryService', () => {
  ...
  beforeEach(() => {
    storageRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'fishswarm-memory-'));
  ...
  it('builds progressive prompt context and supports search/read/debug inspection', async () => {
    ...
    expect(normalizeWorkspaceForAssert(inspected?.sourceWorkspace)).toBe('/repo/a');
  });
  ...
  it('searches all source workspaces when scope is all even with a current cwd', async () => {
    ...
    expect(allResults.some((item) => normalizeWorkspaceForAssert(item.sourceWorkspace) === '/repo/b')).toBe(true);
    ...
    expect(
      workspaceResults.every((item) => item.kind === 'core' || normalizeWorkspaceForAssert(item.sourceWorkspace) === '/repo/a')
    ).toBe(true);
  });
  ...
  it('rejects reading files that escape the memory allowlist through symlinks', async () => {
    ...
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fishswarm-memory-outside-'));
  ...
  it('rejects arbitrary local files even if storageRoot is configured too broadly', () => {
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fishswarm-memory-broad-root-'));
  ...
  it('rejects evalArtifactsRoot values that escape storageRoot before rebuildAll can delete them', async () => {
    const outsideDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'fishswarm-memory-artifacts-escape-')
    );
  ...
  it('rejects readFile when evalArtifactsRoot escapes storageRoot', () => {
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fishswarm-memory-artifacts-read-'));
    const outsideFile = path.join(outsideDir, 'secret.json');
    fs.writeFileSync(outsideFile, '{"secret":true}', 'utf8');
  ...
  it('rejects readFile when evalArtifactsRoot is a filesystem root', () => {
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fishswarm-memory-artifacts-root-'));
    const outsideFile = path.join(outsideDir, 'secret.json');
    fs.writeFileSync(outsideFile, '{"secret":true}', 'utf8');
  ...
  it('rejects readFile when evalArtifactsRoot is a symlink escaping storageRoot', () => {
    if (process.platform === 'win32') return;
    const outsideDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'fishswarm-memory-artifacts-link-target-')
    );
    const outsideFile = path.join(outsideDir, 'secret.json');
    fs.writeFileSync(outsideFile, '{"secret":true}', 'utf8');
  ...
  it('rejects non-existent evalArtifactsRoot paths under escaping symlinks before creating directories', () => {
    if (process.platform === 'win32') return;
    const outsideDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'fishswarm-memory-artifacts-link-parent-')
    );
