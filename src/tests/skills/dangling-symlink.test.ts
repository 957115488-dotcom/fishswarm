...
function supportsDirectorySymlinks(): boolean {
  const probeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fishswarm-symlink-probe-'));
...
describe('isDanglingSymlink', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fishswarm-test-'));
  });
...
describe('dangling symlink cleanup in skill directories', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fishswarm-skills-test-'));
  });
