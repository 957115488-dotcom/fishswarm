...
type TestManagerInternals = {
  ...
  ensureChromeReady: (serverId: string, serverName: string, client: TestMCPClient) => Promise<void>;
};
...
describe('MCPManager', () => {
  ...
  describe('connection timeout', () => {
    ...
    it('prepares Chrome lazily only when a Chrome MCP tool is called', async () => {
      const testManager = asTestManager(manager);
      const chromeClient: TestMCPClient = {
        callTool: vi.fn().mockResolvedValue({ ok: true }),
      };
      const regularClient: TestMCPClient = {
        callTool: vi.fn().mockResolvedValue({ ok: true }),
      };
      testManager.clients = new Map([
        ['chrome-server', chromeClient],
        ['regular-server', regularClient],
      ]);
      testManager.tools = new Map([
        [
          'mcp__Chrome__list_pages',
          {
            name: 'mcp__Chrome__list_pages',
            originalName: 'list_pages',
            description: '',
            inputSchema: { type: 'object', properties: {} },
            serverId: 'chrome-server',
            serverName: 'Chrome',
          },
        ],
        [
          'mcp__Files__read',
          {
            ...
            description: '',
            inputSchema: { type: 'object', properties: {} },
            serverId: 'regular-server',
            serverName: 'Files',
          },
        ],
      ]);

      const ensureChromeReady = vi.fn().mockResolvedValue(undefined);
      testManager.ensureChromeReady = ensureChromeReady;

      await manager.callTool('mcp__Files__read', { path: 'README.md' });
      expect(ensureChromeReady).not.toHaveBeenCalled();

      await manager.callTool('mcp__Chrome__list_pages', {});
      expect(ensureChromeReady).toHaveBeenCalledWith('chrome-server', 'Chrome', chromeClient);
    });
  });
