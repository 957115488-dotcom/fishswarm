...
export function SettingsPanel({ onClose, initialTab = 'api' }: SettingsPanelProps) {
  ...
  const tabs = [
    ...
    {
      id: 'work-habits' as TabId,
      label: t('settings.workHabits', '工作习惯'),
      icon: SlidersHorizontal,
      description: t('settings.workHabitsDesc', '提问偏好与项目决策'),
    },
    {
      id: 'roles' as TabId,
      label: t('settings.roles', '角色管理'),
      icon: UsersRound,
      description: t('settings.rolesDesc', '管理多角色协作、角色手册与自动触发规则'),
    },
    {
      id: 'observability' as TabId,
      label: t('settings.observability', '健康与时间线'),
      icon: Activity,
      description: t('settings.observabilityDesc', '项目健康、改动范围与执行时间线'),
    },
    {
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden px-4 py-6 lg:px-8 lg:py-8">
          <div className="max-w-[860px] w-full min-w-0 mx-auto">
            <div className="">
              ...
              <div className={activeTab === 'work-habits' ? '' : 'hidden'}>
                {viewedTabs.has('work-habits') && <SettingsWorkHabits />}
              </div>
              <div className={activeTab === 'roles' ? '' : 'hidden'}>
                {viewedTabs.has('roles') && <SettingsRoles />}
              </div>
              <div className={activeTab === 'observability' ? '' : 'hidden'}>
                {viewedTabs.has('observability') && <SettingsObservability />}
              </div>
