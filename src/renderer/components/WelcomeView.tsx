...
export function WelcomeView() {
  ...
  const adjustTextareaHeight = useCallback(() => {
    ...
    if (textarea) {
      ...
      // Set max height to 180px, then scroll so the welcome screen stays in view.
      const maxHeight = 180;
  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto flex flex-col items-center px-5 py-5 md:px-8 md:py-6">
      <div className="min-h-full max-w-[840px] w-full flex flex-col justify-center gap-5 md:gap-6 py-3 animate-fade-in">
        <div className="space-y-3 text-center">
          <div className="flex items-center justify-center gap-4">
            <img
              src={welcomeLogoSrc}
              alt={t('welcome.logoAlt')}
              className="app-logo-mark w-14 h-14 md:w-[4.5rem] md:h-[4.5rem] object-contain"
            />
            <div className="text-left">
              <h1 className="text-[2.15rem] md:text-[2.85rem] leading-none font-semibold tracking-[-0.05em] text-text-primary">
                FishSwarm
              </h1>
            </div>
          </div>
          <p className="heading-serif text-[1.08rem] md:text-[1.32rem] font-medium tracking-[-0.02em] text-text-secondary text-center">
            {t('welcome.title')}
          </p>
        </div>
        ...
        <form
          ...
          onDrop={handleDrop}
          className={`rounded-[1.9rem] border border-border-muted bg-background/85 shadow-soft px-5 py-4 space-y-3 transition-colors ${
