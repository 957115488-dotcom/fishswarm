...
import { localizeRoleText } from '../../utils/role-localization';
...
// FishSwarm citation guidance can emit ~[Title](url)~ markers.
...
export const ContentBlockView = memo(function ContentBlockView({
    ...
    () => ({
      ...
      code({ className, children, ...props }: { className?: string; children?: React.ReactNode }) {
        ...
        if (isInline) {
          ...
          return (
            <code
              className="px-1.5 py-0.5 rounded bg-surface-muted text-accent font-mono text-[13px]"
              {...props}
            >
              {children}
            </code>
      ...
      th({ children, style }: { children?: React.ReactNode; style?: React.CSSProperties }) {
        return (
          <th
            className="border border-border px-3 py-2 text-[13px] font-semibold text-text-primary bg-surface-muted"
            style={style}
          >
            {children}
          </th>
      ...
      td({ children, style }: { children?: React.ReactNode; style?: React.CSSProperties }) {
        return (
          <td
            className="border border-border px-3 py-2 text-[13px] text-text-primary"
            style={style}
          >
            {children}
          </td>
  ...
  switch (block.type) {
    case 'text': {
      ...
      const displayText = isUser ? text : localizeRoleText(text, language);
      const normalizedText = normalizeMessageMarkdownText(displayText);
      ...
      if (isUser) {
        return (
          <p className="text-[13px] leading-5 text-text-primary whitespace-pre-wrap break-words text-left">
            {text}
