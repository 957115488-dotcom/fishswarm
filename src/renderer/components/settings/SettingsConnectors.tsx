...
export function SettingsConnectors({ isActive }: { isActive: boolean }) {
  ...
  async function addPresetServer(
    ...
    const resolvedPreset = await window.electronAPI.mcp.createFromPreset(presetKey, false);
    const serverConfig: MCPServerConfig = resolvedPreset || {
    ...
    serverConfig.env = { ...serverConfig.env, ...envOverrides };
  ...
  async function handleDeleteServer(serverId: string) {
    const confirmed = await dialog.confirm({
      message: t('mcp.deleteConnectorConfirm'),
      intent: 'danger',
...
function ServerForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (type === 'stdio') {
      if (!command.trim()) {
        await dialog.alert(t('mcp.commandRequired'));
        return;
      }
      ...
      if (!url.trim()) {
        await dialog.alert(t('mcp.urlRequired'));
        return;
      }
      config.url = url.trim();
    }
