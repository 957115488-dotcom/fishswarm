...
export function SettingsMemory() {
  ...
  const handleRebuildWorkspace = async () => {
    ...
    const confirmed = await dialog.confirm({
      message: t('memory.rebuildConfirm'),
      confirmLabel: t('memory.rebuildWorkspace'),
    });
    if (!confirmed) {
      return;
    }
  ...
  const handleRebuildAll = async () => {
    const confirmed = await dialog.confirm({
      message: t('memory.rebuildAllConfirm', '这会清空并重建全部记忆，是否继续？'),
      intent: 'danger',
      confirmLabel: t('memory.rebuildAll', '重建全部记忆'),
    });
    if (!confirmed) {
      return;
    }
  ...
  const handleClearWorkspace = async () => {
    ...
    const confirmed = await dialog.confirm({
      message: t('memory.clearWorkspaceConfirm'),
      intent: 'danger',
      confirmLabel: t('memory.clearWorkspace'),
    });
    if (!confirmed) {
      return;
    }
  ...
  const handleClearCore = async () => {
    const confirmed = await dialog.confirm({
      message: t('memory.clearCoreConfirm'),
      intent: 'danger',
      confirmLabel: t('memory.clearCore'),
    });
    if (!confirmed) {
      return;
    }
