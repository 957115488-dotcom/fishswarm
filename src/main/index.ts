...
} from './observability/project-learning-service';
import { analyzeChangeScope } from './observability/change-scope-router';
import { buildHealthSummary } from './observability/health-service';
import { createCodeHealthArtifact } from './observability/code-health-service';
import { createBacklogSpecArtifact } from './planning/spec-workflow';
import { createInvestigationArtifact } from './debug/investigation-workflow';
import { createDocumentReleaseCoverageArtifact } from './documentation/document-release-service';
...
import {
  ...
  evaluateShipGate,
} from './shipping/release-gate-service';
import {
...
} from './advanced-qa/advanced-qa-service';
import { createBrowserSkillDraft } from './skills/browser-skillify-service';
import {
  commitBrowserSkillDraft,
  discardBrowserSkillDraft,
  listBrowserSkillRuntime,
  runBrowserSkill,
  setBrowserSkillEnabled,
  testBrowserSkill,
  testBrowserSkillDraft,
  tombstoneBrowserSkill,
} from './skills/browser-skill-runtime';
import {
  clearQuestionPreference,
  getQuestionPolicySnapshot,
  setQuestionPreference,
} from './work-habits/question-policy-store';
import {
  addDecision,
  getDecisionStoreSnapshot,
  redactDecision,
  supersedeDecision,
} from './work-habits/decision-store';
import {
...
function createWindow() {
  ...
  const defaultWindowWidth = Math.max(800, Math.floor(primaryWorkArea.width * (2 / 3)));
  const defaultWindowHeight = Math.max(600, Math.floor(primaryWorkArea.height * (2 / 3)));
...
app
  ...
  .then(async () => {
    ...
    log('=== FishSwarm Starting ===');
    log('Config file:', configStore.getPath());
    log('Is configured:', configStore.isConfigured());
    log('[Runtime] Using FishSwarm agent SDK for all providers');
  ...
  .catch((error) => {
    ...
    dialog.showErrorBox('FishSwarm 启动失败', `${message}\n\n请查看日志获取更多信息。`);
    app.quit();
  });
...
ipcMain.handle('mcp.createFromPreset', (_event, presetKey: string, enabled = false) => {
  try {
    return mcpConfigStore.createFromPreset(presetKey, enabled);
  } catch (error) {
    logError('[MCP] Error creating server from preset:', error);
    return null;
  }
});

ipcMain.handle('context.save', (_event, sessionId: string) => {
  if (!sessionManager) throw new Error('Session manager not initialized');
  return sessionManager.saveContextSnapshot(sessionId);
});

ipcMain.handle('context.restoreLatest', (_event, sessionId: string) => {
  if (!sessionManager) throw new Error('Session manager not initialized');
  return sessionManager.restoreLatestContextSnapshot(sessionId);
});

ipcMain.handle('session.guard.get', (_event, sessionId: string) => {
  if (!sessionManager) throw new Error('Session manager not initialized');
  return sessionManager.getSessionGuard(sessionId);
});

ipcMain.handle('session.guard.set', (_event, sessionId: string, update: SessionGuardUpdate) => {
  if (!sessionManager) throw new Error('Session manager not initialized');
  return sessionManager.setSessionGuard(sessionId, update);
});

ipcMain.handle('session.guard.clear', (_event, sessionId: string) => {
  if (!sessionManager) throw new Error('Session manager not initialized');
  sessionManager.clearSessionGuard(sessionId);
  return { success: true };
});

ipcMain.handle(
  'timeline.list',
  (_event, payload?: { cwd?: string; limit?: number; category?: string }) => {
    return listProjectTimelineEvents({
      cwd: payload?.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd(),
      limit: payload?.limit,
...
ipcMain.handle('workflowArtifacts.list', (_event, payload?: WorkflowArtifactListInput) => {
  const cwd =
...
ipcMain.handle('spec.createArtifact', (_event, payload: BacklogSpecInput) => {
  const cwd =
    payload?.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return createBacklogSpecArtifact({ ...payload, cwd });
});

ipcMain.handle('investigation.createArtifact', (_event, payload: InvestigationInput) => {
  const cwd =
    payload?.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return createInvestigationArtifact({ ...payload, cwd });
});

ipcMain.handle('reviewGate.createArtifact', (_event, payload: ReviewGateInput) => {
  const cwd =
    payload?.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return createReviewGateArtifact({ ...payload, cwd });
});

ipcMain.handle('implementationTasks.aggregate', (_event, payload?: ImplementationTasksInput) => {
  const cwd =
    payload?.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return createImplementationTasksArtifact({ ...payload, cwd });
});

ipcMain.handle('releaseSummary.createArtifact', (_event, payload?: ReleaseSummaryInput) => {
  const cwd =
    payload?.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return createReleaseSummaryArtifact({ ...payload, cwd });
});

ipcMain.handle('shipGate.evaluate', (_event, payload?: ShipGateInput) => {
  const cwd =
    payload?.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return evaluateShipGate({ ...payload, cwd });
});

ipcMain.handle('visualQa.createArtifact', (_event, payload: VisualQaInput) => {
  const cwd =
    payload?.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return createVisualQaArtifact({ ...payload, cwd });
});

ipcMain.handle('canaryMonitor.createArtifact', (_event, payload: CanaryMonitorInput) => {
  const cwd =
    payload?.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return createCanaryMonitorArtifact({ ...payload, cwd });
});

ipcMain.handle(
  'browserSkillEvidence.createArtifact',
  (_event, payload: BrowserSkillEvidenceInput) => {
    const cwd =
      payload?.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
    return createBrowserSkillEvidenceArtifact({ ...payload, cwd });
  }
);

ipcMain.handle('devexAudit.createArtifact', (_event, payload: DevexAuditInput) => {
  const cwd =
    payload?.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return createDevexAuditArtifact({ ...payload, cwd });
});

ipcMain.handle('benchmarkRun.createArtifact', (_event, payload: BenchmarkRunInput) => {
  const cwd =
    payload?.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return createBenchmarkRunArtifact({ ...payload, cwd });
});

ipcMain.handle(
  'browserAuthImport.createSummary',
  (_event, payload: BrowserAuthImportSummaryInput) => {
    const cwd =
      payload?.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
    return createBrowserAuthImportSummaryArtifact({ ...payload, cwd });
  }
);

ipcMain.handle('learnings.list', (_event, payload?: { cwd?: string; limit?: number }) => {
...
ipcMain.handle('learnings.search', (_event, payload?: ProjectLearningSearchInput) => {
  const cwd =
    payload?.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return searchProjectLearnings({ ...payload, cwd });
});
...
ipcMain.handle('learnings.stats', (_event, payload?: { cwd?: string; workspaceKey?: string }) => {
  const cwd =
    payload?.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return getProjectLearningStats({ ...payload, cwd });
});

ipcMain.handle(
  'learnings.maintenanceReport',
  (_event, payload?: { cwd?: string; workspaceKey?: string }) => {
    const cwd =
      payload?.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
    return buildProjectLearningMaintenanceReport({ ...payload, cwd });
  }
);

ipcMain.handle('learnings.exportMarkdown', (_event, payload?: ProjectLearningExportInput) => {
  const cwd =
    payload?.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return exportProjectLearningsMarkdown({ ...payload, cwd, format: 'markdown' });
});

ipcMain.handle('learnings.prune', (_event, payload?: ProjectLearningPruneInput) => {
  const cwd =
    payload?.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return pruneProjectLearnings({ ...payload, cwd });
});

ipcMain.handle('changeScope.analyze', (_event, cwd?: string) => {
  const workspace = cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  const report = analyzeChangeScope(workspace);
  appendProjectTimelineEvent({
    cwd: workspace,
    category: 'change_scope',
    event: 'change_scope.analyzed',
    source: 'change-scope-router',
    status: 'ok',
    summary: `${report.files.length} changed files; recommended skills: ${report.recommendedSkills.join(', ') || 'none'}`,
    metadata: {
...
ipcMain.handle('health.summary', async (_event, cwd?: string) => {
  const workspace = cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  const skills = skillsManager ? await skillsManager.listSkills() : [];
  const summary = buildHealthSummary({
    cwd: workspace,
    isConfigured: configStore.isConfigured(),
    mcpServers: mcpConfigStore.getServers(),
    skills,
    memoryEnabled: configStore.get('memoryEnabled') !== false,
  });
  appendProjectTimelineEvent({
    cwd: workspace,
    category: 'health',
    event: 'health.summary_generated',
    source: 'health-service',
    status: summary.score >= 75 ? 'ok' : summary.score >= 50 ? 'info' : 'error',
    summary: `Health score ${summary.score}`,
    metadata: { checks: summary.checks },
  });
  return summary;
});

ipcMain.handle('codeHealth.snapshot', (_event, cwd?: string) => {
  const workspace = cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return createCodeHealthArtifact(workspace);
});

ipcMain.handle('documentRelease.coverage', (_event, cwd?: string) => {
  const workspace = cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return createDocumentReleaseCoverageArtifact(workspace);
});

ipcMain.handle('browserSkillify.createDraft', (_event, payload: BrowserSkillifyInput) => {
  const workspace =
    payload.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return createBrowserSkillDraft({ ...payload, cwd: workspace });
});

ipcMain.handle('browserSkills.list', (_event, cwd?: string) => {
  const workspace = cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return listBrowserSkillRuntime(workspace);
});

ipcMain.handle('browserSkills.testDraft', (_event, payload: { cwd?: string; stageId: string }) => {
  const workspace =
    payload.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return testBrowserSkillDraft(workspace, payload.stageId);
});

ipcMain.handle(
  'browserSkills.commitDraft',
  (_event, payload: { cwd?: string; stageId: string }) => {
    const workspace =
      payload.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
    return commitBrowserSkillDraft(workspace, payload.stageId, 'project');
  }
);

ipcMain.handle(
  'browserSkills.discardDraft',
  (_event, payload: { cwd?: string; stageId: string }) => {
    const workspace =
      payload.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
    return discardBrowserSkillDraft(workspace, payload.stageId);
  }
);

ipcMain.handle(
  'browserSkills.setEnabled',
  (_event, payload: { cwd?: string; name: string; enabled: boolean }) => {
    const workspace =
      payload.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
    return setBrowserSkillEnabled(workspace, payload.name, payload.enabled);
  }
);

ipcMain.handle('browserSkills.remove', (_event, payload: { cwd?: string; name: string }) => {
  const workspace =
    payload.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return tombstoneBrowserSkill(workspace, payload.name);
});

ipcMain.handle('browserSkills.test', (_event, payload: { cwd?: string; name: string }) => {
  const workspace =
    payload.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return testBrowserSkill(workspace, payload.name);
});

ipcMain.handle(
  'browserSkills.run',
  (_event, payload: { cwd?: string; name: string; timeoutMs?: number }) => {
    const workspace =
      payload.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
    return runBrowserSkill(workspace, payload.name, { timeoutMs: payload.timeoutMs });
  }
);

ipcMain.handle('questionPolicy.snapshot', (_event, cwd?: string) => {
  const workspace = cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return getQuestionPolicySnapshot(workspace);
});
...
ipcMain.handle(
  ...
  (_event, payload?: { cwd?: string; questionId?: string }) => {
    const workspace =
      payload?.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
    return clearQuestionPreference(workspace, payload?.questionId);
  }
);

ipcMain.handle('decisions.snapshot', (_event, cwd?: string) => {
  const workspace = cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return getDecisionStoreSnapshot(workspace);
});

ipcMain.handle('decisions.add', (_event, payload: AddDecisionInput) => {
  const workspace =
    payload.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return addDecision({ ...payload, cwd: workspace, source: payload.source || 'user' });
});

ipcMain.handle('decisions.supersede', (_event, payload: { cwd?: string; id: string }) => {
  const workspace =
    payload.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return supersedeDecision(workspace, payload.id, 'user');
});

ipcMain.handle('decisions.redact', (_event, payload: { cwd?: string; id: string }) => {
  const workspace =
    payload.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return redactDecision(workspace, payload.id, 'user');
});
...
ipcMain.handle(
  ...
  (_event, payload?: { cwd?: string; sessionId?: string; limit?: number }) => {
    const workspace =
      payload?.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
    return getRoleRuntimeSnapshot(workspace, payload?.sessionId, payload?.limit);
  }
);
...
ipcMain.handle('roles.candidates.accept', (_event, payload: SaveRoleCandidateInput) => {
  const workspace =
    payload.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return acceptRoleCandidate(workspace, { ...payload, cwd: workspace });
});

ipcMain.handle('roles.candidates.reject', (_event, payload: RejectRoleCandidateInput) => {
  const workspace =
    payload.cwd || currentWorkingDir || configStore.get('defaultWorkdir') || process.cwd();
  return rejectRoleCandidate(workspace, { ...payload, cwd: workspace });
});
...
ipcMain.handle('logs.export', async () => {
  try {
    ...
    const result = await dialog.showSaveDialog(mainWindow!, {
      title: 'Export Logs',
      defaultPath: `fishswarm-logs-${new Date().toISOString().split('T')[0]}.zip`,
      filters: [
