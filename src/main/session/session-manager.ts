...
} from '../context/context-state-service';
import {
  sessionGuardStore,
  type SessionGuardState,
  type SessionGuardUpdate,
} from './session-guard-store';
...
export class SessionManager {
  ...
  private contextStateService = new ContextStateService();
  ...
  private createAgentRunner(): void {
    ...
    log('[SessionManager] Using FishSwarm agent runner');
  }
  ...
  async deleteSession(sessionId: string): Promise<void> {
    ...
    sessionGuardStore.clear(sessionId);
  ...
  saveContextSnapshot(sessionId: string): ContextSnapshot {
    const session = this.loadSession(sessionId);
    if (!session) {
      throw new Error(`Session not found: ${sessionId}`);
    }
    const snapshot = this.contextStateService.saveSnapshot(session, this.getMessages(sessionId));
    appendProjectTimelineEvent({
      cwd: session.cwd,
      category: 'context',
      event: 'context.snapshot_saved',
      source: 'session-manager',
      status: 'ok',
      summary: `Saved context snapshot for ${session.title}`,
      metadata: {
        sessionId,
        snapshotId: snapshot.id,
        branch: snapshot.branch,
        gitStatusLines: snapshot.gitStatus ? snapshot.gitStatus.split(/\r?\n/).length : 0,
      },
    });
    return snapshot;
  }

  restoreLatestContextSnapshot(sessionId: string): ContextSnapshot | null {
    const session = this.loadSession(sessionId);
    if (!session) {
      throw new Error(`Session not found: ${sessionId}`);
    }
    return this.contextStateService.restoreLatest(session);
  }

  getSessionGuard(sessionId: string): SessionGuardState {
    return sessionGuardStore.get(sessionId);
  }

  setSessionGuard(sessionId: string, update: SessionGuardUpdate): SessionGuardState {
    const session = this.loadSession(sessionId);
    if (!session) {
      throw new Error(`Session not found: ${sessionId}`);
    }
    const freezeRoot = update.freezeRoot === undefined ? session.cwd : update.freezeRoot;
    return sessionGuardStore.set(sessionId, { ...update, freezeRoot });
  }

  clearSessionGuard(sessionId: string): void {
    sessionGuardStore.clear(sessionId);
  }
