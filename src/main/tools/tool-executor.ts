...
export class ToolExecutor {
  ...
  private assertInsideMount(targetPath: string, mounts: MountedPath[]): string {
    ...
    let realPath: string;
    try {
      realPath = isUncPath(normalizedTarget)
        ? normalizedTarget
        : fs.existsSync(normalizedTarget)
  ...
  async writeFile(sessionId: string, filePath: string, content: string): Promise<void> {
    try {
      ...
      sessionGuardStore.assertWriteAllowed(sessionId, pathToWrite);
  ...
  async editFile(
    ...
    try {
      ...
      sessionGuardStore.assertWriteAllowed(sessionId, resolvedPath);
  ...
  private async read(virtualPath: string, context: ExecutionContext): Promise<ToolResult> {
    try {
      ...
      sessionGuardStore.assertWriteAllowed(context.sessionId, realPath);
  ...
  private async bash(command: string, context: ExecutionContext): Promise<ToolResult> {
    ...
    try {
      ...
      sessionGuardStore.assertCommandAllowed(context.sessionId, command, cwd);
    } catch (error) {
      return {
        success: false,
