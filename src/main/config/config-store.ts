...
export async function getPiAiModelPresets(): Promise<typeof PROVIDER_PRESETS> {
  ...
  try {
    ...
    for (const [providerKey, curated] of Object.entries(PI_AI_CURATED)) {
      ...
      const fallbackModels = new Map(preset.models.map((model) => [model.id, model]));
      const registryModelById = new Map(registryModels.map((model) => [model.id, model]));
      const picked = curated.pick.map((id) => {
        const reg = registryModelById.get(id);
        const fallback = fallbackModels.get(id);
        return { id, name: reg?.name || fallback?.name || id };
      });
...
export class ConfigStore {
  ...
  constructor() {
    ...
    this.store = createEncryptedStoreWithKeyRotation<AppConfigRecord>({
      stableKey: 'fishswarm-config-stable-v1',
      legacyKeys: [
        'fishswarm-config-v1',
        ...getLegacyDerivedKeyHexes({
          moduleDirname: __dirname,
          stableSeed: 'fishswarm-config-stable-v1',
          legacySeed: 'fishswarm-config-v1',
          salt: 'fishswarm-config-salt',
        }),
      ],
  ...
  applyToEnv(): void {
    ...
    delete process.env.FISHSWARM_WORKDIR;
    ...
    if (projectedConfig.defaultWorkdir) {
      process.env.FISHSWARM_WORKDIR = projectedConfig.defaultWorkdir;
      // Backward compatibility for older FishSwarm builds and user scripts.
