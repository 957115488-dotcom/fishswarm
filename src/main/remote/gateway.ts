...
import { sanitizeRemotePrompt } from '../security/content-security';
...
export class RemoteGateway extends EventEmitter {
  ...
  private validatePublicExposurePolicy(): void {
    const tunnelEnabled = this.config.tunnel?.enabled === true;
    const publicBind = this.config.bind === '0.0.0.0';
    if ((tunnelEnabled || publicBind) && this.config.auth.mode === 'open') {
      throw new Error(
        'Remote gateway cannot use auth mode "open" while bound publicly or tunnel-enabled. Use pairing, allowlist, or token auth.'
      );
    }
  }

  private constantTimeTokenEqual(candidate: string | undefined, expected: string | undefined): boolean {
  ...
  async start(): Promise<void> {
    ...
    try {
      this.validatePublicExposurePolicy();
  ...
  private async handleChannelMessage(message: RemoteMessage): Promise<void> {
    ...
    if (message.content.type === 'text' && message.content.text) {
      const scan = sanitizeRemotePrompt(message.content.text);
      if (scan.verdict === 'block') {
        writeSecurityAuditEvent({
          type: 'remote.prompt_injection_blocked',
          severity: 'block',
          source: `remote:${message.channelType}`,
        ...
        await this.sendToChannel({
          ...
          content: {
            type: 'text',
            text: 'Message blocked by FishSwarm content security policy.',
          },
          replyTo: message.id,
        });
        return;
      ...
      if (scan.verdict === 'warn') {
        writeSecurityAuditEvent({
          type: 'remote.content_warning',
          severity: 'warn',
          source: `remote:${message.channelType}`,
          reasons: scan.reasons,
          metadata: { channelId: message.channelId, senderId: message.sender.id },
        });
      }
    }
  ...
  private handleWSAuth(client: WSClient, message: WSMessage): void {
    ...
    if (this.config.auth.mode === 'token') {
      if (this.constantTimeTokenEqual(token, this.config.auth.token)) {
        client.authenticated = true;
        client.scopes = ['remote:message'];
        this.sendWSMessage(client.ws, {
          type: 'auth_result',
  ...
  private async handleWSClientMessage(client: WSClient, message: WSMessage): Promise<void> {
    try {
      ...
      const scan = sanitizeRemotePrompt(text || '');
      if (scan.verdict === 'block') {
        writeSecurityAuditEvent({
          type: 'remote.prompt_injection_blocked',
          severity: 'block',
          source: 'remote:websocket',
          reasons: scan.reasons,
          metadata: { clientId: client.id, ip: client.ip },
        });
        this.sendWSMessage(client.ws, {
          type: 'error',
          payload: { error: 'Message blocked by FishSwarm content security policy.' },
          requestId: message.requestId,
        });
        return;
      }
