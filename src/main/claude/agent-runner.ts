 ...
 *
 * Responsibilities:
 * - Runs AI conversations via the FishSwarm agent SDK (createAgentSession)
...
import { sessionGuardStore } from '../session/session-guard-store';
import {
...
const ROLE_SUBCALLS_ENABLED = process.env.FISHSWARM_ROLE_SUBCALLS !== '0';
const LIMA_INSTANCE_NAME =
  process.env.FISHSWARM_LIMA_INSTANCE_NAME ||
  process.env.OPEN_COWORK_LIMA_INSTANCE_NAME ||
  'fishswarm-sandbox';
...
export function getColdStartHistoryTokenBudget(
  provider: string | undefined,
  routeProtocol: string,
  contextWindow: number
): number {
  const normalizedProvider = provider || '';
  const historyBudgetRatio = normalizedProvider === 'ollama' && contextWindow < 16384 ? 0.15 : 0.3;
  const rawBudget = Math.floor(contextWindow * historyBudgetRatio);

  if (normalizedProvider === 'ollama') {
    return rawBudget;
  }

  if (routeProtocol === 'openai') {
    return Math.min(rawBudget, 12000);
  }

  return Math.min(rawBudget, 24000);
}
          ...
          return {
            ...
            details:
              normalizedResult.images.length > 0
                ? { fishSwarmImages: normalizedResult.images }
                : undefined,
          };
        } catch (err: unknown) {
...
function buildRoleOrchestrationPrompt(result: RoleRuntimeDryRunResult): string {
  if (result.routed.roles.length === 0) return '';
  const roleLines = result.routed.roles.map((role) => {
    const reasons = result.routed.reasons[role.id] || [];
    const reasonText = reasons.length > 0 ? reasons.join('; ') : 'Matched role routing rules.';
...
function buildRoleRunResultsPrompt(execution: RoleRuntimeExecutionResult): string {
  ...
  if (results.length === 0 && execution.userVisibleSummary) {
    return [
      '## Role Collaboration State',
      execution.userVisibleSummary,
      'Explain the next user-visible step briefly. Do not claim that a blocked or approval-required candidate role executed work.',
    ].join('\n');
  }
  ...
  const roleLines = results.map((result, index) => {
    const topFindings = result.findings
      ...
      .join('; ');
    const topActions = result.nextActions
      .slice(0, 2)
      .map((action) => `${action.owner}: ${action.action}`)
      .join('; ');
    return [
      `${index + 1}. ${result.roleName} (${result.status}): ${truncateRoleText(result.summary, 260)}`,
      topFindings ? `   findings: ${topFindings}` : '',
      result.decisions.length > 0 ? `   decision candidates: ${result.decisions.length}` : '',
      topActions ? `   next actions: ${topActions}` : '',
    ]
      .filter(Boolean)
      .join('\n');
  });
  ...
  return [
    ...
    hasReworkGate
      ? `Blocked/rework roles: ${reworkResults
          .map(
            (result) =>
              `${result.roleName} (${result.status}): ${truncateRoleText(result.summary, 180)}`
          )
          .join(' | ')}`
      : '',
    ...roleLines,
...
function buildVisibleRoleCollaborationSummary(execution: RoleRuntimeExecutionResult): string {
  if (execution.results.length === 0 && execution.userVisibleSummary) {
    return ['**角色协作**', `- ${execution.userVisibleSummary}`].join('\n');
  }
  return '';
}
...
function roleLifecycleTraceTitle(event: RoleLifecycleEvent): string {
  switch (event.status) {
    case 'gap_detected':
      return '发现角色能力缺口';
    case 'incubating_role':
      return '正在孵化候选角色';
    case 'candidate_ready':
      return `${event.roleName} 候选角色可临时上线`;
    case 'candidate_blocked':
      return `${event.roleName} 候选角色被阻断`;
    case 'approval_required':
      return `${event.roleName} 候选角色等待确认`;
    case 'mounting_handbook':
      return `挂载 ${event.roleName} 角色手册`;
    case 'online':
      return `${event.roleName} 已上线`;
    case 'working':
      return `${event.roleName} 工作中`;
    case 'returned':
      return `${event.roleName} 已返回`;
    case 'validating':
      return `${event.roleName} 验收中`;
    case 'accepted':
    ...
    case 'failed':
      return `${event.roleName} 失败`;
    case 'queued':
    default:
      return `${event.roleName} 等待上线`;
  }
}
...
${hints.join('\n')}
  ...
  constructor(
    ...
    log('[ClaudeAgentRunner] Initialized with FishSwarm agent SDK');
  ...
  private installPermissionHook(piSession: PiAgentSession, sessionId: string): void {
    ...
    const runSessionGuard = (toolName: string, input: Record<string, unknown>) => {
      const normalizedName = toolName.toLowerCase();
      const command = typeof input.command === 'string' ? input.command : undefined;
      if (command || normalizedName.includes('bash') || normalizedName.includes('command')) {
        const cwdInput = typeof input.cwd === 'string' ? input.cwd : undefined;
        const mounts = this.pathResolver.getMounts(sessionId);
        const baseRoot = mounts[0]?.real;
        const cwd =
          cwdInput && path.isAbsolute(cwdInput)
            ? cwdInput
            : cwdInput && baseRoot
              ? path.join(baseRoot, cwdInput)
              : baseRoot;
        sessionGuardStore.assertCommandAllowed(sessionId, command ?? '', cwd);
      }
      ...
      const writeLike =
        ...
        normalizedName.includes('multiedit');
      if (writeLike && pathInput) {
        const mounts = this.pathResolver.getMounts(sessionId);
        const baseRoot = mounts[0]?.real;
        const targetPath = path.isAbsolute(pathInput)
          ? pathInput
          : baseRoot
            ? path.join(baseRoot, pathInput)
            : pathInput;
        sessionGuardStore.assertWriteAllowed(sessionId, targetPath);
      }
    };
    const getPolicyCwd = (input: Record<string, unknown>): string | undefined => {
      const cwdInput = typeof input.cwd === 'string' ? input.cwd : undefined;
      if (cwdInput && path.isAbsolute(cwdInput)) return cwdInput;
      const mounts = this.pathResolver.getMounts(sessionId);
      const baseRoot = mounts[0]?.real;
      if (cwdInput && baseRoot) return path.join(baseRoot, cwdInput);
      return baseRoot;
    };
    ...
    agent.setBeforeToolCall(
      ...
      async (ctx: any, signal?: AbortSignal): Promise<any> => {
        ...
        try {
          runSessionGuard(toolName, input);
        } catch (guardError) {
          const reason =
            guardError instanceof Error ? guardError.message : 'Tool blocked by session guard';
          log(`[ClaudeAgentRunner] Tool '${toolName}' blocked by session guard: ${reason}`);
        ...
        if (decision === 'ask') {
          ...
          const policyCwd = getPolicyCwd(input);
          const permissionQuestion = buildPermissionQuestion({
            cwd: policyCwd,
            sessionId,
            toolName,
            input,
          });
          const policy = evaluateQuestionPolicy(permissionQuestion);
          if (policy.action === 'auto_decide' && policy.choice === 'allow') {
            log(
              `[QuestionPolicy] Auto-allowed '${toolName}' for session ${sessionId}: ${policy.reason}`
            );
          ...
          recordQuestionAnswer({
            cwd: policyCwd,
            questionId: permissionQuestion.questionId,
            summary: permissionQuestion.summary,
            choice: result === 'deny' ? 'deny' : 'allow',
            result,
            source: 'permission-dialog',
            setPreference: result === 'allow_always' ? 'never-ask' : undefined,
          });
    ...
    try {
      ...
      if (sandbox.isLima && sandbox.limaStatus?.instanceRunning && workingDir) {
        ...
        if (syncResult.success) {
          ...
          try {
            ...
            execFileSync(
              'limactl',
              ['shell', LIMA_INSTANCE_NAME, '--', 'mkdir', '-p', sandboxSkillsPath],
              {
                encoding: 'utf-8',
                timeout: 10000,
              }
            ...
            const copiedSkills = execFileSync(
              'limactl',
              ['shell', LIMA_INSTANCE_NAME, '--', 'ls', sandboxSkillsPath],
              {
                encoding: 'utf-8',
                timeout: 10000,
              }
      ...
      if (!cachedSession) {
        ...
        if (historyMessages.length > 0) {
          ...
          const historyTokenBudget = getColdStartHistoryTokenBudget(
            provider,
            configProtocol,
            contextWindow
          );
      ...
      if (roleOrchestrationPrompt) {
        contextualPrompt = `${roleOrchestrationPrompt}\n\n${contextualPrompt}`;
      }
      ...
      if (this.mcpManager) {
          ...
          for (const config of allConfigs) {
            try {
              ...
              if (config.type === 'stdio') {
                ...
                if (!imageCapable) {
                  serverEnv.FISHSWARM_DISABLE_IMAGE_TOOL_OUTPUT = '1';
                ...
                const hasPlaceholders = resolvedArgs.some(
                  (arg) =>
                    ...
                    arg.includes('{GSTACK_BROWSE_SERVER_PATH}')
                );
                ...
                if (hasPlaceholders) {
                  ...
                  } else if (config.name === 'GStack_Browse' || config.name === 'GStack Browse') {
                    presetKey = 'gstack-browse';
                  }
        ...
        if (
          (process.env.FISHSWARM_LOG_SDK_MESSAGES_FULL ||
...
- Use WebSearch/WebFetch only when Chrome MCP is unavailable or the user explicitly asks for generic web search.
      ...
      if (cachedSession) {
        ...
        const resourceLoader = new DefaultResourceLoader({
          ...
          appendSystemPrompt: fishSwarmAppendPrompt,
        });
  ...
  private async prepareRoleRuntimePrompt(
    session: Session,
    prompt: string,
    ...
    try {
      if (ROLE_SUBCALLS_ENABLED) {
        ...
        const results = await runRolesWithModel(
          {
            ...
            sessionId: session.id,
            taskText,
            context: buildShortRoleContext(session, existingMessages),
            emit: (event) => {
        ...
        const visibleRoleSummary =
          results.results.length === 0 ? buildVisibleRoleCollaborationSummary(results) : '';
        if (visibleRoleSummary) {
          this.sendMessage(session.id, {
            id: uuidv4(),
            sessionId: session.id,
            role: 'assistant',
            content: [{ type: 'text', text: visibleRoleSummary }],
            timestamp: Date.now(),
          });
        }
      ...
      const failureEvent: RoleLifecycleEvent = {
        ...
        runId: 'role-runtime',
        roleId: 'role-runtime',
        roleName: 'Role Runtime',
        status: 'failed',
        summary,
      };
      this.sendToRenderer({ type: 'role.lifecycle', payload: failureEvent });
      this.publishRoleTraceEvent(session.id, failureEvent, roleTraceIds);
      try {
        appendProjectTimelineEvent({
          cwd: workingDir,
          category: 'role',
          event: 'role.runtime_failed',
          source: 'agent-runner',
          status: 'error',
          summary,
  ...
  private publishRoleTraceEvent(
    ...
    let traceId = traceIds.get(event.runId);
    if (!traceId) {
      traceId = `role-${event.runId}`;
      traceIds.set(event.runId, traceId);
      this.sendTraceStep(sessionId, {
        id: traceId,
        type: 'thinking',
        status: roleLifecycleTraceStatus(event.status),
        title: roleLifecycleTraceTitle(event),
    ...
    this.sendTraceUpdate(sessionId, traceId, {
      status: roleLifecycleTraceStatus(event.status),
      title: roleLifecycleTraceTitle(event),
      content: event.summary,
    });
