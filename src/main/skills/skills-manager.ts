...
function getSkillCreatedAt(stat: fs.Stats): number {
...
function sortSkillsByCreatedAt(skills: Skill[]): Skill[] {
...
export class SkillsManager {
      ...
      try {
        ...
        for (const dir of skillDirs) {
          ...
          const skill: Skill = {
            ...
            type: 'builtin',
            enabled: true,
            createdAt: getSkillCreatedAt(stat),
          };
  ...
  private getDeletedSkillsPath(globalSkillsPath = this.getGlobalSkillsPath()): string {
    return path.join(globalSkillsPath, '.deleted-skills.json');
  }

  private normalizeSkillNameForDeletedStore(name: string): string {
    return name.trim().toLowerCase();
  }

  private readDeletedSkillNames(globalSkillsPath = this.getGlobalSkillsPath()): Set<string> {
    const deletedSkillsPath = this.getDeletedSkillsPath(globalSkillsPath);
    try {
      const parsed = JSON.parse(fs.readFileSync(deletedSkillsPath, 'utf8')) as DeletedSkillsStore;
      return new Set(
        Array.isArray(parsed.names)
          ? parsed.names
              .map((name) =>
                typeof name === 'string' ? this.normalizeSkillNameForDeletedStore(name) : ''
              )
              .filter(Boolean)
          : []
      );
  ...
  private writeDeletedSkillNames(
    ...
    if (names.size === 0) {
      try {
        fs.rmSync(deletedSkillsPath, { force: true });
      } catch {
    ...
    fs.writeFileSync(
      deletedSkillsPath,
      JSON.stringify({ names: Array.from(names).sort() }, null, 2),
      'utf8'
    );
  }

  private markSkillNameDeleted(name: string, globalSkillsPath = this.getGlobalSkillsPath()): void {
    const normalizedName = this.normalizeSkillNameForDeletedStore(name);
    if (!normalizedName) return;
    const names = this.readDeletedSkillNames(globalSkillsPath);
    names.add(normalizedName);
    this.writeDeletedSkillNames(names, globalSkillsPath);
  }

  private clearDeletedSkillName(name: string, globalSkillsPath = this.getGlobalSkillsPath()): void {
    const normalizedName = this.normalizeSkillNameForDeletedStore(name);
    if (!normalizedName) return;
    const names = this.readDeletedSkillNames(globalSkillsPath);
    if (!names.delete(normalizedName)) {
      return;
    }
    this.writeDeletedSkillNames(names, globalSkillsPath);
  }
  ...
  private async importUserSkills(globalSkillsPath: string): Promise<void> {
    ...
    const deletedSkillNames = this.readDeletedSkillNames(globalSkillsPath);
    ...
    for (const entry of entries) {
      ...
      if (
        metadata &&
        deletedSkillNames.has(this.normalizeSkillNameForDeletedStore(metadata.name))
      ) {
        log(`[Skills] Skipping user skill previously deleted in FishSwarm: ${metadata.name}`);
        continue;
      }
  ...
  private async loadSkillsFromDirectory(
    ...
    const deletedSkillNames =
      source === 'global' ? this.readDeletedSkillNames(dir) : new Set<string>();
      ...
      for (const entry of entries) {
        ...
        if (stat.isDirectory()) {
          ...
          if (fs.existsSync(skillMdPath)) {
            ...
            if (!metadata) continue;
            if (deletedSkillNames.has(this.normalizeSkillNameForDeletedStore(metadata.name))) {
              log(`[Skills] Skipping deleted global skill: ${metadata.name}`);
              continue;
            }
        ...
        // Also support legacy .json config files
          ...
          try {
            ...
            if (!config.name) {
              logWarn(`[Skills] Skipping legacy skill config without name: ${entryPath}`);
              continue;
            }
   ...
   */
  getAllSkills(): Skill[] {
    return sortSkillsByCreatedAt(this.deduplicateSkills(Array.from(this.loadedSkills.values())));
  }
  ...
  async listSkills(filter?: {
    ...
    return sortSkillsByCreatedAt(skills);
  }
  ...
  private async copySkillToGlobal(
    ...
    const targetPath = path.join(globalSkillsPath, targetFolderName);
  ...
  private getUniqueSkillFolderName(globalSkillsPath: string, baseName: string): string {
    const safeBaseName = baseName.replace(/[^a-zA-Z0-9._ -]/g, '-').trim() || 'skill';
    for (let index = 1; index <= 100; index += 1) {
      const candidate = `${safeBaseName}-${Date.now().toString(36)}-${index}`;
      const candidatePath = path.join(globalSkillsPath, candidate);
      try {
    ...
    throw new Error(`Could not choose a free skill folder name for: ${baseName}`);
  }
  ...
  private makeWritableRecursive(targetPath: string): void {
    ...
    try {
      fs.chmodSync(targetPath, lstat.isDirectory() ? 0o700 : 0o600);
    } catch (error) {
      logWarn(`[Skills] Could not make path writable before delete: ${targetPath}`, error);
    }
    ...
    try {
      ...
      logWarn(`[Skills] Could not enumerate path before delete: ${targetPath}`, error);
      return;
    }

    for (const entry of entries) {
      this.makeWritableRecursive(path.join(targetPath, entry));
    }
  }
  ...
  private removeDirectoryRecursive(targetPath: string): void {
    ...
    const removeOptions: fs.RmOptions = {
      recursive: true,
      force: true,
    ...
    try {
      fs.rmSync(targetPath, removeOptions);
      return;
    } catch (error) {
  ...
  private getGlobalSkillFolderName(skill: Skill): string {
    const globalPrefix = 'global-';
    if (skill.id.startsWith(globalPrefix)) {
      const folderName = skill.id.slice(globalPrefix.length);
  ...
  async installSkill(skillPath: string): Promise<Skill> {
    ...
    const globalSkillsPath = this.getGlobalSkillsPath();
    this.clearDeletedSkillName(metadata.name, globalSkillsPath);
    ...
    let targetFolderName = metadata.name;
    ...
    if (isDanglingSymlink(targetPath)) {
      ...
      try {
        ...
        targetFolderName = this.getUniqueSkillFolderName(globalSkillsPath, metadata.name);
        logWarn(
          `[Skills] Could not remove existing skill directory, installing into ${targetFolderName}: ${targetPath}`,
          error
        );
      }
    }
    ...
    try {
      await this.copySkillToGlobal(skillPath, metadata.name, targetFolderName);
    } catch (error) {
      if (targetFolderName !== metadata.name || !isRetriableRemoveError(error)) {
        throw error;
      }
      targetFolderName = this.getUniqueSkillFolderName(globalSkillsPath, metadata.name);
      logWarn(
        `[Skills] Could not install skill into canonical folder, retrying in ${targetFolderName}: ${targetPath}`,
        error
      );
      await this.copySkillToGlobal(skillPath, metadata.name, targetFolderName);
    }
  ...
  async uninstallSkill(skillId: string): Promise<void> {
    ...
    if (skill.type === 'custom') {
      // Use app-specific skills directory to avoid conflicts with user settings
      const globalSkillsPath = this.getGlobalSkillsPath();
      this.markSkillNameDeleted(skill.name, globalSkillsPath);
      const skillDir = path.join(globalSkillsPath, this.getGlobalSkillFolderName(skill));
      ...
      if (fs.existsSync(skillDir)) {
        try {
          ...
          logWarn(
            `[Skills] Could not delete skill directory, keeping deleted marker: ${skillDir}`,
            error
          );
        }
      }
