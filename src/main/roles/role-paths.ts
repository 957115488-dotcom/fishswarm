...
export function getRolesRoot(): string {
  return process.env.FISHSWARM_ROLES_ROOT || path.join(os.homedir(), '.fishswarm', 'roles');
}

export function getRolesWorkspaceDir(cwd?: string): string {
  const workspaceKey = getWorkspaceKey(cwd);
  return path.join(getRolesRoot(), 'workspaces', workspaceKey);
}

export function ensureRolesWorkspaceDir(cwd?: string): string {
  const dir = getRolesWorkspaceDir(cwd);
