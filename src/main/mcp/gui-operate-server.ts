...
function getFishSwarmEnv(primary: string, legacy: string): string | undefined {
  return process.env[primary] || process.env[legacy];
}

// Get FishSwarm data directory for persistent storage
// Use platform-appropriate paths:
// - macOS: ~/Library/Application Support/fishswarm
// - Windows: %APPDATA%/fishswarm
const FISHSWARM_DATA_DIR =
  getFishSwarmEnv('FISHSWARM_DATA_DIR', 'OPEN_COWORK_DATA_DIR') ||
  (PLATFORM === 'win32'
    ? path.join(process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming'), 'fishswarm')
    : path.join(os.homedir(), 'Library', 'Application Support', 'fishswarm'));

// Directory for storing GUI operate files (screenshots, etc.)
const GUI_OPERATE_DIR = path.join(FISHSWARM_DATA_DIR, 'gui_operate');
...
const GUI_APPS_DIR = path.join(FISHSWARM_DATA_DIR, 'gui_apps');
...
function getResourcesDirCandidates(): string[] {
  ...
  const envResources = getFishSwarmEnv('FISHSWARM_RESOURCES_PATH', 'OPEN_COWORK_RESOURCES_PATH');
...
async function resolveCliclickPath(): Promise<string | null> {
  ...
  const envOverride = getFishSwarmEnv('FISHSWARM_CLICLICK_PATH', 'OPEN_COWORK_CLICLICK_PATH');
...
async function resolvePythonExec(): Promise<PythonExec | null> {
  ...
  // 1) Explicit override (useful for debugging)
  const envPython = getFishSwarmEnv('FISHSWARM_PYTHON_PATH', 'OPEN_COWORK_PYTHON_PATH');
  const envPythonHome = getFishSwarmEnv('FISHSWARM_PYTHON_HOME', 'OPEN_COWORK_PYTHON_HOME');
  ...
  try {
    ...
    if (/Accessibility privileges not enabled/i.test(result.stderr || '')) {
      const hint =
        ...
        '- System Settings → Privacy & Security → Accessibility：允许 FishSwarm\n' +
        '- 如果是终端运行：允许 Terminal/iTerm\n' +
        '- 授权后请重启 FishSwarm 再重试\n';
      throw new Error(
    ...
    const hint =
      '\n\nmacOS 权限提示 / Permissions:\n' +
      '- System Settings → Privacy & Security → Accessibility：允许 FishSwarm\n' +
      '- System Settings → Privacy & Security → Automation：允许 FishSwarm 控制 “System Events”\n';
...
async function takeScreenshot(
  ...
  try {
    ...
    const hint =
      '\n\nmacOS 权限提示 / Permissions:\n' +
      '- System Settings → Privacy & Security → Screen Recording：允许 FishSwarm\n' +
...
async function takeScreenshotForDisplay(
  ...
  const disableImageOutput =
    getFishSwarmEnv(
      'FISHSWARM_DISABLE_IMAGE_TOOL_OUTPUT',
      'OPEN_COWORK_DISABLE_IMAGE_TOOL_OUTPUT'
    ) === '1';
...
async function callVisionAPIWithTimeout(
  ...
  if (isOpenAICompatible) {
    ...
    if (isOpenRouter) {
      headers['HTTP-Referer'] = 'https://github.com/FishSwarm/fishswarm';
      headers['X-Title'] = 'FishSwarm';
    }
