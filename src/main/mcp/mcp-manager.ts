...
import { connectWithOAuthRetry, FishSwarmMcpOAuthProvider } from './mcp-oauth';
import { log, logError, logWarn, logCtx, logCtxError, logTiming } from '../utils/logger';
import { getDefaultShell } from '../utils/shell-resolver';
import { scanUntrustedText, wrapUntrustedContent } from '../security/content-security';
import { redactUnknown } from '../security/redact';
import { writeSecurityAuditEvent } from '../security/security-audit-log';
import { evaluateMcpScopePolicy } from '../security/token-scope';
import { sanitizeUnknownModelOutput } from '../observability/output-sanitizer';
...
export class MCPManager {
  ...
  private oauthProviders: Map<string, { provider: FishSwarmMcpOAuthProvider; serverUrl: string }> =
    new Map();
  ...
  private async resolvePreferredNpxPath(pathEnv: string | undefined): Promise<string> {
    ...
    if (process.platform === 'win32') {
      ...
      if (!preferredNpxPath) {
        throw new Error(
          'npx is not available. Install Node.js so FishSwarm can use your system npx.cmd, or reinstall the app to restore the bundled runtime.'
        );
      }
  ...
  private async connectServerInternal(config: MCPServerConfig): Promise<void> {
    ...
    if (config.type === 'stdio') {
      ...
      const isBuiltinServer =
        ...
        config.name === 'GStack_Browse' ||
        config.name === 'GStack Browse';
      const isOldConfig =
      ...
      args = args.map((arg) => {
        ...
        if (arg === '{GSTACK_BROWSE_SERVER_PATH}') {
          return this.getMcpServerPath('gstack-browse-server.ts');
        }
        return arg;
      });
      ...
      if (app.isPackaged && isBuiltinServer) {
        ...
        env.FISHSWARM_RESOURCES_PATH = process.resourcesPath || '';
        // Backward compatibility for MCP servers from older FishSwarm builds.
        env.OPEN_COWORK_RESOURCES_PATH = process.resourcesPath || '';
      }

      env.FISHSWARM_TIMELINE_ROOT =
        env.FISHSWARM_TIMELINE_ROOT || path.join(app.getPath('userData'), 'project-timeline');
  ...
  private getOrCreateStreamableHttpOAuthProvider(
    ...
    const provider = new FishSwarmMcpOAuthProvider({
  ...
  private sanitizeMcpToolResult(result: unknown, serverName: string, toolName: string): unknown {
    const normalized = sanitizeUnknownModelOutput(result);
    const redacted = redactUnknown(normalized);
    if (redacted.redacted) {
      writeSecurityAuditEvent({
        type: 'mcp.result_redacted',
        severity: 'warn',
        source: 'mcp',
        reasons: redacted.findings,
        metadata: { serverName, toolName },
      });
    }

    if (!redacted.value || typeof redacted.value !== 'object' || Array.isArray(redacted.value)) {
      return redacted.value;
    }

    const maybeResult = redacted.value as { content?: unknown };
    if (!Array.isArray(maybeResult.content)) {
      return redacted.value;
    }

    let hadWarnings = false;
    const content = maybeResult.content.map((item) => {
      ...
      const scan = scanUntrustedText(block.text);
      ...
      return {
        ...block,
        text: wrapUntrustedContent(scan.sanitizedText, `mcp:${serverName}:${toolName}`),
      };
    });

    if (hadWarnings) {
      writeSecurityAuditEvent({
        type: 'mcp.result_content_warning',
        severity: 'warn',
        source: 'mcp',
        metadata: { serverName, toolName },
      });
  ...
  async callTool(toolName: string, args: Record<string, unknown>): Promise<unknown> {
    ...
    const scopePolicy = evaluateMcpScopePolicy(tool.serverName, actualToolName);
    writeSecurityAuditEvent({
      type: 'mcp.tool_call',
      severity: scopePolicy.allowed ? 'info' : 'block',
      source: 'mcp',
      reasons: scopePolicy.reason ? [scopePolicy.reason] : undefined,
      metadata: {
    ...
    if (!scopePolicy.allowed) {
      throw new Error(
        scopePolicy.reason || `MCP tool blocked by security policy: ${actualToolName}`
      );
    }

    const redactedArgs = redactUnknown(args);
    if (redactedArgs.redacted) {
      writeSecurityAuditEvent({
        type: 'mcp.arguments_redacted',
        severity: 'warn',
        source: 'mcp',
        reasons: redactedArgs.findings,
        metadata: { serverName: tool.serverName, toolName: actualToolName },
      });
    }
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        if (currentTool.serverName.toLowerCase().includes('chrome')) {
          await this.ensureChromeReady(currentTool.serverId, currentTool.serverName, client);
        }
        ...
        return this.sanitizeMcpToolResult(result, tool.serverName, actualToolName);
      } catch (error: unknown) {
        lastError = error;
