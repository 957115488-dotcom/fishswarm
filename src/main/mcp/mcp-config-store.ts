...
export const MCP_SERVER_PRESETS: Record<string, Omit<MCPServerConfig, 'id' | 'enabled'> & { requiresEnv?: string[]; envDescription?: Record<string, string> }> = {
  ...
  'gstack-browse': {
    ...
    type: 'stdio',
    command: 'node',
    args: ['{GSTACK_BROWSE_SERVER_PATH}'], // Path will be resolved at runtime (compiled JS in production)
    env: {
      GSTACK_BROWSE_BIN: '',
    },
    requiresEnv: [],
    envDescription: {
      GSTACK_BROWSE_BIN:
        'Optional path to the compiled gstack browse binary. If empty, FishSwarm searches bundled resources/gstack-browse and common skill install paths.',
    },
  },
};
...
class MCPConfigStore {
  ...
  getServers(): MCPServerConfig[] {
    ...
    const resolvedServers = servers.map((server) => this.resolveServerPathPlaceholders(server));
    if (JSON.stringify(servers) !== JSON.stringify(resolvedServers)) {
      this.store.set('servers', resolvedServers);
    }
  ...
  saveServer(config: MCPServerConfig): void {
    config = this.resolveServerPathPlaceholders(config);
    const servers = this.getServers();
  ...
  /**
   * Get the path to the GStack Browse MCP adapter file
   */
  private getGStackBrowseServerPath(): string | null {
    return this.getMcpServerPath('gstack-browse-server.ts');
  }

  private resolveServerPathPlaceholders(config: MCPServerConfig): MCPServerConfig {
    if (!config.args?.some((arg) => arg.includes('_SERVER_PATH}'))) {
      return config;
    }
    ...
    return {
      ...
      args: config.args.map((arg) => {
        ...
        if (arg === '{GSTACK_BROWSE_SERVER_PATH}') {
          return this.getGStackBrowseServerPath() || arg;
        }
        return arg;
      }),
    };
