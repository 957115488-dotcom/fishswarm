...
interface ConnectWithOAuthOptions<TTransport extends OAuthTransport> {
  ...
  createTransport: (provider: FishSwarmMcpOAuthProvider) => TTransport;
  provider: FishSwarmMcpOAuthProvider;
}

function buildClientMetadata(redirectUrl: string): OAuthClientMetadata {
  return {
    client_name: 'FishSwarm MCP Connector',
...
export class FishSwarmMcpOAuthProvider implements OAuthClientProvider {
  clientMetadataUrl?: string;
...
export async function createOAuthCallbackListener(
  ...
  const server = createServer((request, response) => {
    ...
    if (authorizationCode) {
      ...
      response.end(
        '<html><body><h1>Authorization complete</h1><p>You can return to FishSwarm now.</p><script>setTimeout(() => window.close(), 1200);</script></body></html>'
      );
