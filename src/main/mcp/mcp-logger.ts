...
function getLogsDirectory(): string {
  ...
  if (platform === 'darwin') {
    ...
    appDataDir = path.join(os.homedir(), 'Library', 'Application Support', 'fishswarm');
  } else if (platform === 'win32') {
    // Windows: %APPDATA%/fishswarm
    appDataDir = path.join(process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming'), 'fishswarm');
  } else {
    // Linux: ~/.config/fishswarm
    appDataDir = path.join(os.homedir(), '.config', 'fishswarm');
  }
